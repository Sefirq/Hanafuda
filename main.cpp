        ...
        while(if_play) {
            ...
            if (whose_turn) {
               ...
               /* cout << "Player 1's move!" << endl << "If you can't find a proper move, please type '0'" << endl;
                ...
                while (!if_correct_move) {
                         cout << "Choose cards from the same month - first your card, then card from the table (1-"
                         << table_of_game->length() << ")" << endl;
                    cin >> chosen_card_from_hand;
                    if(!chosen_card_from_hand){
                        if_any_move_possible = check_if_any_move_possible(table_of_game, player1);
                        if(if_any_move_possible){
                            cout<<"There is at least one possible move"<<endl;
                            continue;
                        }
                        else{
                            cout << "No possible moves" << endl << "Choose a card to discard it to the table (1-" << player1->length()<<")"<<endl;
                            if_correct_move = false;
                            while(!if_correct_move) {
                                cin >> chosen_card_from_hand;
                                if_correct_move = try_to_discard(player1, table_of_game, chosen_card_from_hand);
                            }
                            break;
                        }
                    }
                    cin >> chosen_card_from_table;
                    cout<<chosen_card_from_hand<<chosen_card_from_table<<endl;
                    if_correct_move = check_if_correct(table_of_game, player1, chosen_card_from_hand,
                                                       chosen_card_from_table);
                }
                //now the 2nd move in a turn
                card top_card = deck->pop_a_card();
                top_card.print();
                if_correct_move = false;
                while(!if_correct_move){
                    cout << "Try to find card from the same month as this one. If it is not possible, type '0'" << endl;
                    cout << "Type a number from 1 to " << table_of_game->length() << endl;
                    cin >> chosen_card_from_table;//
                    if(!chosen_card_from_table){
                        if_any_move_possible = check_if_any_move_possible(table_of_game, player1);
                        if(if_any_move_possible){
                            ...
                            cout << "No possible moves" << endl << "Placing the card on the table..." << endl;
                            table_of_game->add_card(top_card);
                            break;
                        }
                    }//
                    else{
                        if_correct_move = check_second_move(table_of_game, player1, top_card, chosen_card_from_table);
                    }
                }
                whose_turn = false;
                if_correct_move = false;
                ...
                while (!if_correct_move) {
                    ...
                    cin >> chosen_card_from_hand;
                    if(!chosen_card_from_hand){
                        if_any_move_possible = check_if_any_move_possible(table_of_game, player2);
                        if(if_any_move_possible){
                            cout<<"There is at least one possible move"<<endl;
                            continue;
                        }
                    ...
                    cin >> chosen_card_from_table;
                    if_correct_move = check_if_correct(table_of_game, player2, chosen_card_from_hand,
                                                       chosen_card_from_table);
                }
                whose_turn = true;
                if_correct_move = false;
